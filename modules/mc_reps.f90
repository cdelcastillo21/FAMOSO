...
contains
    ...
    subroutine mc_e_thresh(f_yx, nx, ny, e_thresh)
        ...
        do i=1,n_elements
            f_yx(f_sort(1,i),f_sort(2,i)) = f_sort(3,i)
        end do
    ...
    ! If scale flag is set, the given gd is first fitted to the given 
    ! particle data so that all the particles fit in the grid. Otherwise, 
    ! partiles outside the grid are ignored when contsructing the distribution
    ! function. The distributuion function in s_rep is normalized
    function mc_xy_to_f (p_rep, gd)
        ...
        ! Declare Auxiliary Variables
        integer :: i, j, nx, ny, nd, np, flag, np_in_grid
        ...
        ! Normalize f
        s_rep%f_yx = s_rep%f_yx / mc_srep_norm(s_rep)!(gd%delta_x_i(1)*gd%delta_x_i(2)*real(np_in_grid))
    ...
    subroutine mc_fit_grid_to_particles(curr_gd, p_rep, max_gd)
        type(grid), intent(INOUT) :: curr_gd
        type(particle_rep), intent(IN) :: p_rep 
        type(grid), optional, intent(IN) :: max_gd

        integer :: i,j,np,nd
        real(8), dimension(:), allocatable :: new_upper, new_lower
        ...
        ! Find new upper and lower bounds that fit the given p_rep 
        do j=1,np
            do i=1,nd
        ...
        ! If max gd specified make sure new upper/lower bounds don't go past it
        if (present(max_gd)) then
            do i=1,nd
                if (new_upper(i) > max_gd%upper_bnds(i) ) then
                    new_upper(i) = max_gd%upper_bnds(i)
                end if

                if (new_lower(i) < max_gd%lower_bnds(i) ) then
                    new_lower(i) = max_gd%lower_bnds(i)
                end if
            end do
        end if

        ! Rescale grid to new upper and lower bounds. 
        call mc_grid_rescale(curr_gd, new_upper, new_lower)
